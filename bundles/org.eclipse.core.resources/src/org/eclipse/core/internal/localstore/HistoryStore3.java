...
public class HistoryStore3 implements IHistoryStore {
	...
	public HistoryStore3(Workspace workspace, IPath location, int limit) {
	...
	public IFileState addState(IPath key, java.io.File localFile, long lastModified, boolean moveContents) {
		...
		try {
			...
			HistoryBucketIndex currentBucket = (HistoryBucketIndex) tree.getCurrent();
	...
	HistoryBucketIndex createBucketTable() {
		return new HistoryBucketIndex(indexLocation);
	}
